...
//> INSTANCE PROPERTIES
	...
	private ThinletCursorFlasher flashCursor;
	...
	public Thinlet() { // fixed by Mike Hartshorn (javac1.1 bug)
		...
		flashCursor = new ThinletCursorFlasher();
		flashCursor.start();
	}
	...
	private void doLayout(Object component) {
		...
		if (COMBOBOX == classname) {
			...
			if (divider == -1) {
				...
				// FIXME Seems to be a race condition which allows vertical splitpane to collapss to 0 width sometimes...
...
//> THINLET INSTANCE ACCESSORS
	...
	private class ThinletCursorFlasher extends Thread {
		boolean running;
		ThinletCursorFlasher() {
			super("Thinlet Cursor Flasher");
			// Make this thread a daemon in case it is not terminated properly
			// TODO surely would be more sensible to make sure that this terminates properly
